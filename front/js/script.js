fetch ("http://localhost:3000/api/products") //récupérez les données dans l'API
    ...
    .then(function(result){
        ...
        result.forEach(canapes => { // pour chaque canapes, on a le résultat
            console.log(canapes) // on affiche dans la console
            let lien = document.createElement("a") // on crée la variable lien
            lien.href="./product.html?id="+canapes._id // on fait le lien entre le fichier HTML et JS et on récupére l'ID de chaque canapé
            ...
            img.src=canapes.imageUrl //on va chercher imageUrl dans l'API
            article.appendChild (img)  //on ajoute image dans les enfants de "article"
            img.alt=canapes.altTxt //on va chercher altTxt dans l'API
            ...
            title.textContent = canapes.name // on ajoute le texte contenu dans le h3 (API)
            ...
            paragraphe.textContent = canapes.description //on ajoute le texte contenu dans le paragrphe (API)
